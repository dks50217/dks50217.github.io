...
假設我要從 20 首裡挑 8 首排順序，總共有幾種排法？答案是幾百億種以上。一種一種試，電腦大概要算到天荒地老。
...
尤其「接不接得順，要看上一首是什麼」這點很關鍵，讓整題變得超像旅行推銷員問題（TSP），就是業務員要跑完所有城市、每段路有距離，求總路程最短。你把城市換成歌、距離換成接得順不順，幾乎就是同一題。而這種題目機車就機車在這：它是所謂的 NP-hard。講白一點，歌一多就別想又快又保證最佳。這也是為什麼我這個東西頂多算到二十幾首，再多電腦就吃不消了。

所以說穿了，我就是把 DJ 靠手感在做的事，拆成一條一條電腦算得動的規則，再拿課本裡的動態規劃跟 Pareto 前緣去解。
