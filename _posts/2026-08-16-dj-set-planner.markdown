...
對照起來還蠻剛好的：決定歌的先後，在 OR 裡就是排序、排程；從一堆歌先挑再排，叫組合最佳化。最有意思的是「上一首會影響下一首順不順」這種事，OR 裡叫**順序相依成本**，工廠換模、換線在算的就是它。至於要同時顧到嗨、順、高潮這三件會打架的事，那是多目標最佳化；最後吐出一組各有取捨的方案，就是所謂的 **Pareto 效率前緣**。

尤其「接不接得順，要看上一首是什麼」這點很關鍵，讓整題變得超像**旅行推銷員問題（TSP）**，就是業務員要跑完所有城市、每段路有距離，求總路程最短。你把城市換成歌、距離換成接得順不順，幾乎就是同一題。而這種題目機車就機車在這：它是所謂的 **NP-hard**。講白一點，歌一多就別想又快又保證最佳。這也是為什麼我這個東西頂多算到二十幾首，再多電腦就吃不消了。

所以說穿了，我就是把 DJ 靠手感在做的事，拆成一條一條電腦算得動的規則，再拿課本裡的**動態規劃**跟 **Pareto 前緣**去解。
...
至於「沒有唯一答案」那件事，我的做法是**留一整組互不吃虧的方案**。意思是，一套方案只要找不到另一套樣樣都比它強的，就留著。所以最後程式不是丟一個答案給我，而是給我好幾套風格不一樣的歌單，我今天想順一點還是想炸一點，自己挑。
...
它算得動的大概就二十幾首上限，再多真的會爆（前面講過那個排列爆炸是本質問題，躲不掉）。可是真實的 DJ 曲庫動不動就上百上千首，所以比較實際的做法是：先用簡單規則粗篩，把上千首砍到十五到二十首候選，再丟給這個程式去精算順序。
